  ...
  function Materialrueckgabe($bestellung){
    ...
    while (!$materialliste->listenende()) {
      ...
      if ($materialliste->aktueller_index()%2 != 0) {  
        ...
        $pdf->Cell(20, 5, utf8_decode($material->getRueckgabemenge()), "LR", 0, "C", 1);
        if ($material->getVerbrauchsmaterial()) {
          $pdf->Cell(20, 5, "", "LR", 0, "C", 1);
          $pdf->Cell(20, 5, "", "LR", 1, "C", 1);   
        } else {
          $pdf->Cell(20, 5, utf8_decode($material->getRueckgabeDefekt()), "LR", 0, "C", 1);
          $pdf->Cell(20, 5, utf8_decode($material->getAusgabemenge()-$material->getRueckgabemenge()-$material->getRueckgabeDefekt()), "LR", 1, "C", 1);  
        }
      } else {
        // ungerade Zeilen
        $pdf->Cell(110, 5, utf8_decode($material->getBezeichnung()), "LR");
        $pdf->Cell(20, 5, utf8_decode($material->getAusgabemenge()), "LR", 0, "C");
        $pdf->Cell(20, 5, utf8_decode($material->getRueckgabemenge()), "LR", 0, "C");
        if ($material->getVerbrauchsmaterial()) {
          $pdf->Cell(20, 5, "", "LR", 0, "C");
          $pdf->Cell(20, 5, "", "LR", 1, "C");   
        } else {  
          $pdf->Cell(20, 5, utf8_decode($material->getRueckgabeDefekt()), "LR", 0, "C");
          $pdf->Cell(20, 5, utf8_decode($material->getAusgabemenge()-$material->getRueckgabemenge()-$material->getRueckgabeDefekt()), "LR", 1, "C"); 
        }
      }
    }
