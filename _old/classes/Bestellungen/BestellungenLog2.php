...
class BestellungenLog{

/*************************************************************************************************************************************
 * Funktionen                                                                                                                        *
 *************************************************************************************************************************************/

  function AnlassdatenUebenpruefen($daten){
    // benötigte Variablen definieren resp. zurücksetzen
    $fehler = "";
    $return = 1;
    unset($_SESSION["error"]);
     
    // Aufbereiten der eingegebenen Datumsangaben
    $von = strtotime($daten["von"]);
    $bis = strtotime($daten["bis"]);
    $heute = strtotime(date("d.m.Y"));
    
    // Eingegebene Daten zwischenspeichern
    $bestellung = $_SESSION['bestellung'];
	$bestellung->setEinheit($daten["einheit"]);
	$bestellung->setVon($von); 
	$bestellung->setBis($bis); 
	$bestellung->setAnlass($daten["anlass"]); 
	$_SESSION['bestellung'] = $bestellung;
      
    // Eingegebene Daten überprüfen
    if ($daten['einheit'] == ""){
      $fehler = "Es wurde keine Einheit ausgewählt.";
      $return = -1;
    } else if ($daten['anlass'] == ""){
      $fehler = "Es wurde kein Anlass eingegeben.";
      $return = -1;
    } else if ($von > $bis){
      $fehler = "Das Material kann nicht vor der Ausgabe zurückgegeben werden.";
      $return = -1;
    } else if ($von < $heute){
      $fehler = "Das Ausgabedatum kann nicht in der Vergangenheit liegen.";
      $return = -1;
    } else if ($von < $heute+7*24*60*60){
      $fehler = "Die Bestellung ist sehr kurzfristig!";
      $return = -2;
    }
  ...
  function KurzfristigBestaetigen($daten){
    $return = $this->AnlassdatenUebenpruefen($daten);
    if ($return == -2){
      if(isset($daten['kurzfristig']) && $daten['kurzfristig'] == 1){
        $return = 0;
        unset($_SESSION["error"]);
        // todo: Bestätigung Kurzfristig speichern!
      }
    }
     return $return;
  }
