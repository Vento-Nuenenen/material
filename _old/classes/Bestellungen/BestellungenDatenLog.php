 ...
 *************************************************************************************************************************************/
 ...
 function Einheit($id) {
  ...
  function Kontakte($einheit) {
    $aktuell = $_SESSION['benutzer']->getId();
    $i = 1;
      
    $db = connectDB();
    $select = "SELECT * FROM uro_benutzerrollen LEFT JOIN usr_benutzer ON uro_benutzer = usr_id WHERE uro_rolle IN (SELECT `rei_rolle` FROM `rei_rolleneinheiten` WHERE ".
              "`rei_einheit` = $einheit AND `rei_rolle` != 1) AND usr_aktiv = 1";
    ...
    if ($i){
      $benutzer = new Benutzer();
      $benutzer->setId($_SESSION['benutzer']->getId());
	  $benutzer->setBenutzername($_SESSION['benutzer']->getBenutzername());
  ...
  function KategorieBestimmen($keys) {
    $kat = 0;
    foreach ($keys as $key) {
      if (substr($key, 0, 3) == "kat" && substr($key, -2) == "_x"){
        $end = strpos($key, "_");
        $kat = substr($key, 3, $end-3);  
      }
    }
    return $kat;
  }
