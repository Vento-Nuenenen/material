...
/*
SELECT * FROM `bst_bestellung` LEFT JOIN `ein_einheiten` ON `bst_einheit` = `ein_id` LEFT JOIN `usr_benutzer` ON `bst_kontakt` = 1 WHERE (`bst_einheit` = 3 OR `bst_einheit` = 2) AND (bst_status < 4 OR (bst_status >= 4 AND bst_bis > 1470952800))
*/
  ...
  function BestellungenListe($benutzer) {
	...
	$select = "SELECT * FROM `bst_bestellung` LEFT JOIN `ein_einheiten` ON `bst_einheit` = `ein_id` LEFT JOIN usr_benutzer ON bst_kontakt = usr_id".$where." AND (bst_status < 4 OR (bst_status >= 4 AND bst_bis > ".(strtotime(date("d.m.Y"))-1000*24*60*60)."))";
    /*if ($status == 4) {
...
echo"Debug 2.3<br>";
  ...
  function BestellungLoeschen($id) {
    $db = connectDB();
    $sql = "SELECT `bst_status` AS status, MAX(`mab_ausgabe`) AS ausgabe FROM `bst_bestellung` LEFT JOIN `mab_materialbestellung` ON `bst_id` ".
           "= `mab_bestellung` WHERE `mab_bestellung` = ". $id;
    $res = $db->query($sql);
    $check = $res->fetch_object();  
    if ($check->status <=2 && $check->ausgabe == 0)  {
